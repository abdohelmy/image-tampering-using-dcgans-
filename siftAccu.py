# -*- coding: utf-8 -*-
"""
Created on Mon Apr 02 21:50:39 2018
...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 27 11:27:12 2018
...
def compute_transformations(C, p, p1, min_cluster_pts):
    ...
    if c_max > 1:
        for k, j in permutations(list(range(1, c_max+1)), 2):
            z1 = []
            z2 = []
            ...
            if (len(z1) > min_cluster_pts) and (len(z2) > min_cluster_pts):
                ...
                if M is not None:
                    if len(M) != 0:
                        numPoints.append(len(z1))
                        pointsLocation.append(z1)
                        num_gt = num_gt + 1
    
    return num_gt,numPoints,pointsLocation
...
def process_image(img, kp_detector, \
    ...
    if (kp_descriptor == 'SIFT') or (kp_descriptor == 'SIFT'):
        norm = cv2.NORM_L2
    else:
    ...
    if len(p1) == 0:
        ...
        num_gt,numPoints,pointsLocation = compute_transformations(C, p, p1, min_cluster_pts)
...
def generic_experiment(kp_detector, kp_descriptor,Image):
    ''' Run a generic experiment '''
    # Dataset
    #DB = 'MICC-F220'
    #DB = 'testDB'
    #DB_DIR = 'database'
    #FILE_GROUND_TRUTH = 'groundtruthDB_220-dash.txt'
    #FILE_GROUND_TRUTH = 'names2.txt'
    ...
    num_gt,numPoints,pointsLocations = process_image(Image, kp_detector, kp_descriptor, \
            METRIC, TH, MIN_CLUSTER_PTS, True)

    if num_gt >=1:
            #if ground[imagefile] == '1':
            #TP = TP + 1
            #output[imagefile] = 'Fake'
        return 'Fake',numPoints,pointsLocations
            #else:
                #FP = FP + 1
                #output[imagefile] = 'Fake'
                #return 'Fake'
            ...
            #if ground[imagefile] == '1':
             #   FN = FN + 1
              #  output[imagefile] = 'Original'
            #else:
             #   TN = TN + 1
              #  output[imagefile] = 'Original'
          return 'Originial' ,[0],pointsLocations
    ...
    #return acc,output #TPR, FPR , 
   

def all_experiments(Image):
    ...
    for detector in kp_detectors:
        for descriptor in kp_descriptors:
            #try:
                ...
                t0 = time.time()
                #TPR, FPR,acc = generic_experiment(detector,descriptor)
                #acc , output = generic_experiment(detector,descriptor,Image)
                output,numPoints,pointsLocations = generic_experiment(detector,descriptor,Image)
                
                t1 = time.time()
                #print(('Computational time: %f s' % (t1-t0)))
                #print('TPR = %f\nFPR = %f \nacc = %f' % (TPR*100,FPR*100 , acc))
                #print('acc = %f \n' % (acc))
                #print(output)
                #print("number of key points = " + str(numPoints[0]))
                
                return numPoints[0],pointsLocations[0]
                
            #except:
             #   print('%s,%s error' % (detector, descriptor)
#img = cv2.imread("E:/DSC_0812tamp1.jpg",cv2.IMREAD_COLOR)
#img = cv2.imread("C:\Users\Lenovo\Desktop\image forgery\salah's dataset\dataset\img0.jpg",cv2.IMREAD_COLOR)
#all_experiments(img)
