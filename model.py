...
class DCGAN(object):
    def __init__(self, sess, image_size=64, is_crop=False,
                 batch_size=20, sample_size=20, lowres=8,
...
""")
    ...
    def complete(self, config,numbeOfPoints,coordinates,cropX,cropY):
        def make_dir(name):
        ...
        assert(isLoaded)
        ##########################
        data = dataset_files(config.imgs)
        config.imgs = data
        print(len(data))
        ##########################
...
#            x1=[19,19]
#            x2=[61,61]
            for i in range(numbeOfPoints):
                
                x=coordinates[i][0]
                
                x=(math.floor(x))-cropX
                print(x)            
                y=coordinates[i][1]
                y=(math.floor(y))-cropY
                print(y)            
                mask[y-2:y+2:1,x-2:x+2:1,:] = 0
            #mask=mask[278:342 , 82:146]       
            # mask[x1[i]:x2[i]:1,y1[i]:y2[i]:1,:] = 0
